...
![ODデータの可視化](./img/tutorial_007.png)
...
#### ⑪一件明細データの変換
- ICカード履歴データを一件明細データの標準フォーマットに変換します。
- 生成にあたっては件明細データの「乗車日時・降車日時」および「乗車駅（停留所）名・降車駅（停留所）名」を使用します。
- これらの情報から実際に乗車した便を推定し、便コード（trip_id）と乗車／降車停留所連番（stop_sequence）を推定します。
- 推定した trip_id と stop_sequence を基に、GTFSデータと照合し、運行事業者コード（agency_id）・系統ID・経路ID・乗車／降車停留所コードを一件明細データへひも付けることにより、ICカード履歴データを一件明細データの標準フォーマットへと変換します。

![一件明細データの変換](./img/tutorial_012.png)  

#### ⑫乗降データの変換
- ⑪で作成した一件明細データから乗降データを作成します。
- 乗降実績データとは公共交通機関の乗客の乗降実績を記録したCSV形式の電子データです。本プロジェクトにて以下のように様式を定めています。
...
![乗降データの変換](./img/tutorial_013.png)  

#### ⑬ODデータの変換
- ⑪で作成した一件明細データからODデータを作成します。
- ODデータとは	公共交通機関の出発地から目的地まで（Origin-Destination（起点-終点））の輸送量の実績を記録したCSV形式の電子データデータです。本プロジェクトにて以下のように様式を定めています。
