...
from gtfs.utils.one_detailed_od_utils import (
    convert_one_detailed_to_od,
    convert_ridership_records_to_od,
    od_rows_to_csv,
    read_uploaded_file,
)
...
class OneDetailedODServiceError(Exception):
...
class OneDetailedODService:
    ...
    def convert(
        *,
        ...
        OneDetailedODService._assert_scenario_access(user=user, scenario_id=scenario_id)

        if uploaded_file_bytes is not None and uploaded_filename:
            return OneDetailedODService._convert_from_file(
                uploaded_file_bytes=uploaded_file_bytes,
                uploaded_filename=uploaded_filename,
                scenario_id=str(scenario_id),
            )

        return OneDetailedODService._convert_from_ridership_upload(
            user=user,
    ...
    @staticmethod
    def build_metadata_payload(*, result: ODConversionResult) -> dict[str, Any]:
        csv_content = od_rows_to_csv(result.od_rows)
        return {
            "success": True,
            "source_type": result.source_type,
            "total_input_rows": result.total_input_rows,
            "total_output_rows": len(result.od_rows),
    ...
    def _assert_scenario_access(*, user, scenario_id) -> None:
        try:
            ...
            raise OneDetailedODServiceError(
    ...
    def _convert_from_file(
        *,
        ...
        try:
            ...
            raise OneDetailedODServiceError(message=str(e), status_code=400) from e
        ...
        od_rows, errors = convert_one_detailed_to_od(df, scenario_id)
        return ODConversionResult(
            od_rows=od_rows,
            errors=errors,
    ...
    @staticmethod
    def _convert_from_ridership_upload(*, user, ridership_upload_id, scenario_id: str) -> ODConversionResult:
        ...
        od_rows, errors = convert_ridership_records_to_od(records, scenario_id)
        return ODConversionResult(
            od_rows=od_rows,
            errors=errors,
