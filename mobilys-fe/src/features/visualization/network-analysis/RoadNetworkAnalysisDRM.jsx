...
function RoadNetworkAnalysis() {
  const {
    ...
    handleMapClick,
  } = useRoadNetworkAnalysis({ graphType: "drm" });

  const confirmText =
    graphStatus === "built" || graphStatus === "rebuilt"
      ? VISUALIZATION.roadNetworkAnalysisDrm.confirmText.whenBuilt
      : VISUALIZATION.roadNetworkAnalysisDrm.confirmText.otherwise;

  const drmDescription = VISUALIZATION.roadNetworkAnalysisDrm.description;
  return (
    <Box
      sx={{
    ...
    >
      <Typography variant="h4" sx={{ mb: 1, fontWeight: 600, ml: 1 }}>
        {VISUALIZATION.titles.roadNetworkAnalysisDrm}
      </Typography>
      <PageBanner
        text={drmDescription}
        maxLines={1}
      ...
      <VisualizationTwoPaneLayout
        ...
        graphs={
          loadingRes ? (
            <Typography variant="body2" sx={{ mt: 2 }}>
              {VISUALIZATION.common.loading.results}
            </Typography>
          ) : graphData ? (
            <RoadNetworkGraphContainer
              ...
              /** pass the slider minutes to graphs */
              maxMinutes={maxMinutes}
              scenarioName={currentScenarioName}
              screenName={VISUALIZATION.roadNetworkAnalysisDrm.screenName}
            />
          ) : (
            <Paper
              variant="outlined"
        ...
        right={
          <RoadNetworkMap
            ...
            scenarioId={typeof selectedScenario === "object" ? selectedScenario.id : selectedScenario}
            center={coords}
