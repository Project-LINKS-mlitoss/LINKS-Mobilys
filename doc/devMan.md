...
本章では、AWS EC2（Amazon Linux 2023）上で本ツールをデプロイする手順を説明します。
...
5. `ec2-user` で sudo なし Docker 実行できるようにグループ追加します。
...
7. 再度 `EC2 Instance Connect` で接続し、バージョン確認を実行します。
...
`mobilys-be/.env` を編集し、以下をEC2 のIPに更新します。

```bash
ALLOWED_HOSTS="localhost,127.0.0.1,＜EC2_IP＞"
CORS_ALLOWED_ORIGINS="http://＜EC2_IP＞:3000"
```

`docker-compose.yml` を編集し、`mobilys_fe` の build args を EC2 のIPに変更します。

```yaml
VITE_API_BASE_URL: http://＜EC2_IP＞:8000/api
VITE_API_USERS_BASE_URL: http://＜EC2_IP＞:8000/api
```

人口メッシュデータや関連データインポートを利用する場合は、`mobilys-be/.env` の以下も設定してください。
...
4. 全てのコンテナが起動したらブラウザから以下のURLを開き、本ツールにアクセスしてください。<br>

URL：http://＜EC2_IP＞:3000<br>
...
> **2回目以降の起動**
>
> 2回目以降の起動では以下を実行してください

```bash
docker-compose up
```

<br>DBデータを含むローカル環境を完全に初期化する場合：
...
### Docker/Docker Composeのインストール方法<br>
